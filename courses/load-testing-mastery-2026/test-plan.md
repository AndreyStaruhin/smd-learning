...
### Терминология: «реверс-СМЭВ»

В проекте исторически используется термин **«реверс-СМЭВ»** — это направление **«гейт → наш сервис»** через TLS HTTP. Название появилось, когда направление обращения «развернулось»: раньше наш сервис сам отправлял запросы в гейт (исходящее направление), теперь гейт отправляет запросы нам (входящее направление). С точки зрения нашего сервиса это **обычный входящий HTTP-API**: контроллеры под `/smev/api/v1/*` (`PersonsSmevController`, `OrganizationsSmevController`) принимают POST'ы от гейта, валидируют тело, кладут стартовое событие в очередь.

Важно не путать:

| Что | Термин | Где в коде |
|---|---|---|
| Входящий HTTP-API сервиса (гейт → нас) | **«реверс-СМЭВ-API»** | `PersonsSmevController`, `OrganizationsSmevController` |
| Исходящие SOAP-запросы в СМЭВ-адаптер | «СМЭВ-вызовы» / «адаптер» | `AdapterRequestExecutor.sendRequest()`, `getResponse()` |
| Опрос адаптера на входящие нотификации (отзыв согласия и т.п.) | «пуллер нотификаций» — **side feature, не путать с реверс-СМЭВ** | `AbstractEsiaNotificationPuller`, `AbstractPermissionsNotificationPuller` |

В нагрузочных сценариях этого плана точкой инжекции «честной» нагрузки является именно **реверс-СМЭВ-API** — один POST в `/smev/api/v1/PersonalData` равен одной задаче в очереди. Пуллер нотификаций — отдельный, нишевый канал, который в основной нагрузке практически не участвует.
...
**Не входит в scope** этой итерации: мульти-инстансная конфигурация, soak-тест 24+ часа, полный end-to-end с реальным логином пользователя в ЕСИА, сценарий пуллера нотификаций (`AbstractPermissionsNotificationPuller` — нишевый side-channel, не основная нагрузка).

> **Замечание по предыдущей формулировке:** более ранние редакции этого плана исключали из scope «реверс-СМЭВ». Это была ошибка терминологии — под «реверс-СМЭВ» имелся в виду именно входящий API (см. §1), который, наоборот, **является основной точкой инжекции** нагрузки на воркеры. После уточнения с командой формулировка скорректирована.
...
[Test machine]                   [Local VM]                         [Internet]
  Gatling                                    
   ├─ /rest/api/v1/cb*  ──HTTP──▶ tomcat (dp-service)                ТР ЕСИА
   └─ /smev/api/v1/*    ──HTTP──▶  ↑                                  ▲
       (реверс-СМЭВ-API,            │                                  │
        вход «гейт→нас»)            ├──SOAP──▶ WireMock (СМЭВ adapter)
...
Две независимые точки инжекции с тестовой машины:
- **`/rest/api/v1/cb*`** — синхронный REST-сценарий A (банк инициирует через ТР ЕСИА).
- **`/smev/api/v1/*`** — реверс-СМЭВ-API; сюда «как гейт» бьёт сценарий B и сценарий C для прямой нагрузки на воркеров.

### Стек инструментов

- **Gatling** (тестовая машина) — нагружает две точки входа сервиса:
  - REST-цепочку `/rest/api/v1/cb*` (сценарий A, синхронный путь);
  - реверс-СМЭВ-API `/smev/api/v1/*` (сценарии B-HTTP и C — основная нагрузка на воркеров).
  Hosted на отдельной машине, чтобы не смешивать нагрузку с сервисом.
- **WireMock** (на VM рядом с сервисом) — моки для адаптера СМЭВ (`SmevOptions.AdapterUri`, обратное направление: наш сервис → СМЭВ) и Gate-out (`Gates.AbsGateUri`). Отвечают фиксированно + задержка ~150 мс. **Не путать** с реверс-СМЭВ-API: WireMock-стаб СМЭВ-адаптера моделирует **исходящие** SOAP-вызовы из нашего сервиса; реверс-СМЭВ-API — **входящий** HTTP-API нашего сервиса.
...
Цель: чистая скорость воркеров ФЛ, без шума от REST-цепочки `/cb`.

### Способы инжекции (выбираем под задачу)

| Способ | Как работает | Когда использовать | Преимущество |
|---|---|---|---|
| **B-HTTP** (рекомендуемый по умолчанию) | Gatling шлёт `POST /smev/api/v1/PersonalData` (реверс-СМЭВ-API), каждый POST = одна задача в очередь | Когда нужно мерить пропускную способность нижней части пути «как в проде»: гейт→наш HTTP→`SmevService.process`→`StartScenarioEvent`→очередь | Реалистичный путь, контролируемый RPS, проходит через все валидации/маппинги |
| **B-SQL** | TRUNCATE + SQL-скрипт вставляет N готовых задач в `tasks` со стадией `GetResponse` | Когда нужно изолировать **только** часть «воркер↔БД↔адаптер», без затрат на парсинг/валидацию входящего HTTP | Самый чистый замер скорости стадии-обработки, без накладных расходов входа |

В большинстве случаев предпочтителен **B-HTTP**: он даёт «честную» пропускную способность того же пути, по которому идёт прод-нагрузка (банк → гейт → наш сервис). **B-SQL** — для изолированной диагностики, когда B-HTTP уже показал предел, и мы хотим понять, упирается ли он в стадию-обработку или в HTTP-вход.

### Подготовка (B-HTTP)

1. `TRUNCATE tasks, history, esia_operations RESTART IDENTITY`.
2. Запускаем сервис с `queue.executor.mode=person` — чтобы ЮЛ-пул не конкурировал.
3. Адаптер СМЭВ и Gate-out — WireMock с задержкой 150 мс.
4. Gatling-сценарий шлёт `POST /smev/api/v1/PersonalData` с минимально-валидным телом и заголовками `X-Bank-Id`, `X-Request-Id` (см. `PersonsSmevController:56-87`). Тело может быть фиксированным шаблоном — мы не тестируем валидацию.

> **Тонкость HTTP-входа.** На каждый POST `/smev/api/v1/PersonalData` сервис до возврата `200 OK` **синхронно** выполняет `SmevService.handle()` (`logic/smev/SmevService.java:48`), включая `sendAdapterRequest(...)` (`:71`) — это **исходящий SOAP-вызов в СМЭВ-адаптер** (на стенде — WireMock с задержкой ~150 мс). Только после этого создаётся задача со стадией `GetResponse`, которую воркер затем обработает асинхронно (там — ещё один SOAP-вызов в адаптер). Один POST → **два WireMock-вызова в разных точках жизненного цикла**: один в HTTP-входе, второй в воркере. Поэтому HTTP-вход реверс-СМЭВ-API **не «прозрачен»**: его латентность включает ≥ 150 мс WireMock-времени, и при подсчёте теоретического потолка `T_task` нужно различать «время обработки задачи воркером» и «время цикла POST → задача создана».

### Подготовка (B-SQL — альтернатива для изолированного замера)
...
3. Останавливаем сервис, запускаем с `queue.executor.mode=person`.
...
## 6.1 Сценарий C — комбинированная нагрузка (REST + реверс-СМЭВ-API одновременно)

**Зачем нужен этот сценарий:** A покрывает только REST-слой `/cb` и не нагружает воркеры (запросы успевают пройти HTTP-фазу до того, как очередь успевает забиться). B изолированно меряет воркеров, но без HTTP-конкуренции за CPU/Hikari. Сценарий C — это **единственный сценарий, в котором асинхронная часть нагружается так же, как в проде**: трафик от Gatling параллельно идёт по двум точкам входа (REST `/cb` + реверс-СМЭВ-API `/smev/api/v1/*`), воркеры разгребают обе цепочки задач, обе подсистемы делят один CPU и один Hikari-пул.

**Конфигурация:**

- Gatling гонит **две Gatling-сессии параллельно**:
  - сессия A — полный цикл REST `/cb` (как в §5);
  - сессия B-HTTP — реверс-СМЭВ-API `/smev/api/v1/PersonalData` (как в §6).
  Между прогонами **`truncate` не делаем** — это steady-state, задачи копятся естественно.
...
- [ ] Отдельная БД; скрипт `seed_tasks.sql` для сценария B; `reset.sql` (TRUNCATE `tasks`, `history`, `esia_operations`) **по умолчанию между прогонами** — конкретная итерация может выбрать иной tear-down (см. план соответствующей итерации; например, `LOAD_TEST_ITERATION_1.md` §8).
...
- `core/src/main/java/ru/modernsys/dp/api/cb/CbEsiaController.java` — REST endpoints для сценария A (`/rest/api/v1/cb*`).
...
- **Реверс-СМЭВ-API** (входящий HTTP, гейт→наш сервис, точка инжекции для сценариев B-HTTP и C):
  - `core/src/main/java/ru/modernsys/dp/api/smev/PersonsSmevController.java` — `/smev/api/v1/{PersonalData, PersonalDataByCenter, CustomerPermissionList}`.
  - `core/src/main/java/ru/modernsys/dp/api/smev/OrganizationsSmevController.java` — `/smev/api/v1/{ReceivingOrgDoc, ClaimOrgGrant}`.
  Каждый POST → `SmevService.process()` → `StartScenarioEvent` → задача в очереди.
