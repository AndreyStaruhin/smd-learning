...
## Сценарий 4. Позиционный Code Review
...
1. "Классический Spring-архитектор" (стиль Rod Johnson / Craig Walls):
   чистота слоёв, явность конфигурации, декомпозиция на beans.

2. "Современный прагматик-минималист" (стиль Josh Long):
   избыточность, конвенции vs явность, современные фичи 
   (records, virtual threads, sealed).

3. "Ops-инженер" (SRE, который будет запускать в проде):
   startup time, memory, observability, конфигурируемость,
   graceful shutdown, health checks.

Не сглаживай разногласия — они и есть ценность.
В конце — какое замечание наиболее приоритетное.
```

**Вариация — контр-кейс:**
```
Saga pattern через @EventListener поверх Spring.
Ответь как "Функциональный скептик": чем хуже Temporal/Flowable или explicit FSM?
Где сломается при росте сложности?
```

---

## Сценарий 5. Экзамен модуля

**Когда:** в конце каждого модуля. Блокирует переход.

```
Экзамен Модуля 5 (Persistence).

Твоя роль: senior tech lead на onboarding нового инженера.
Я 2 дня на проекте, ты даёшь задачу из бэклога.

Задача:
"В Methodolog нужна фича: отчёт 'Top 10 активных игроков за период' —
по числу созданных игр со статусом ACTIVE.
Scheduled-задача раз в час, сохранение в БД.
Прод: 10M+ игр без degradation."

Формат:
1. Ты даёшь задачу.
2. 60 минут на решение.
3. Я показываю код.
4. Разбор:
   - Работает? (запусти тесты)
   - Идиоматично для SB3? 
   - Спроси меня о 3 решениях как senior.
